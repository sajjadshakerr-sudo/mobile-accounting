<template>
    <div class="page active relative-page">
        ...
        <div v-show="!isFormOpen" class="top-action-bar mb-md">
            <div class="stats-banner-card" style="background: linear-gradient(135deg, #3b82f6, #2563eb);" @click="showStatsModal = true">
                <div class="d-flex align-center gap-sm">
                    ...
                    <div class="sbc-text">
                        <strong class="sbc-title">گزارش مالی فروش</strong>
                        <span class="sbc-subtitle">مشاهده سودها و مطالبات</span>
                    </div>
                </div>
                <div class="sbc-arrow">
        ...
        <div v-if="isFormOpen" class="form-container-anim">
            <div class="card p-md" style="padding-bottom: 24px;">
                <h2 class="mb-md text-lg font-900" style="color: var(--text-dark);">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-left: 6px; color: var(--primary);"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                    {{ editId ? 'ویرایش فاکتور فروش' : 'ثبت فاکتور جدید' }}
                </h2>
                <form @submit.prevent="saveSale" class="compact-form modern-form">
                    
                    <div class="modern-section">
                        <div class="ms-header text-primary-dark">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                            <span>اطلاعات اصلی</span>
                        </div>
                        <div class="form-row">
                            <div class="form-group"><label>تاریخ ثبت:</label><input type="text" v-model="form.date" class="form-control text-center" required dir="ltr"></div>
                            <div class="form-group"><label>کالا / مدل (اجباری):</label><input type="text" v-model="form.itemName" class="form-control text-right" placeholder="مثال: قاب A54" required></div>
                        </div>
                        <div class="form-row mb-0">
                            <div class="form-group mb-0"><label>نام مشتری (اختیاری):</label><input type="text" v-model="form.customerName" class="form-control text-right" placeholder="نام کامل"></div>
                            <div class="form-group mb-0"><label>موبایل (اختیاری):</label><input type="tel" v-model="form.phone" class="form-control text-center" placeholder="0912..." dir="ltr"></div>
                        </div>
                    </div>
                    ...
                    <div class="modern-section mt-sm">
                        <div class="ms-header" style="color: #059669;">
                            ...
                            <span>مبالغ و مالی</span>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label>قیمت خرید / تمام‌شده:</label>
                                <div class="input-with-currency">
                                    <input type="text" :value="formatNumber(form.buyPrice)" @input="e => handleNumberInput('buyPrice', e)" class="form-control text-center font-800" required dir="ltr" placeholder="0">
                                    <span class="currency-addon">تومان</span>
                                </div>
                            </div>
                            <div class="form-group">
                                <label>مبلغ فاکتور (قیمت فروش):</label>
                                <div class="input-with-currency">
                                    <input type="text" :value="formatNumber(form.sellPrice)" @input="e => handleNumberInput('sellPrice', e)" class="form-control text-center text-success-dark font-900 input-highlight" required dir="ltr" placeholder="0">
                        ...
                        <div class="form-row mb-0">
                            <div class="form-group mb-0">
                                ...
                                <div class="input-with-currency">
                                    <input type="text" :value="formatNumber(form.partnerShare)" @input="e => handleNumberInput('partnerShare', e)" class="form-control text-center font-800" placeholder="0" dir="ltr">
                                    <span class="currency-addon">تومان</span>
                                </div>
                            </div>
                            <div class="form-group mb-0">
                                <label>یادداشت:</label>
                                <input type="text" v-model="form.notes" class="form-control" placeholder="توضیحات کوتاه...">
                            </div>
                        </div>
                    </div>
                    
                    <div class="form-group mt-lg">
                        <label class="d-block mb-sm text-sm font-800 text-dark">نوع تسویه فاکتور:</label>
                        <div class="payment-type-selector">
                            <label class="pt-option" :class="{'active-cash': form.paymentType === 'cash'}"><input type="radio" v-model="form.paymentType" value="cash" style="display: none;"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="20" height="12" rx="2"></rect><circle cx="12" cy="12" r="2"></circle><path d="M6 12h.01M18 12h.01"></path></svg><span>نقدی کامل</span></label>
                            <label class="pt-option" :class="{'active-credit': form.paymentType === 'credit'}"><input type="radio" v-model="form.paymentType" value="credit" style="display: none;"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg><span>نسیه (ناقص)</span></label>
                            <label class="pt-option" :class="{'active-install': form.paymentType === 'installment'}"><input type="radio" v-model="form.paymentType" value="installment" style="display: none;"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="5" width="20" height="14" rx="2"></rect><line x1="2" y1="10" x2="22" y2="10"></line></svg><span>فروش اقساطی</span></label>
                        </div>
                    </div>
                    
                    <div v-if="form.paymentType === 'credit'" class="dynamic-section theme-credit">
                        <strong class="section-badge">جزئیات فروش نسیه</strong>
                        <div class="form-group mb-0 mt-sm">
                        ...
                        <div class="form-row mt-md mb-0">
                            <div class="form-group mb-0"><label>IMEI 1 (اختیاری):</label><input type="text" v-model="form.imei1" class="form-control dir-ltr text-center" maxlength="50"></div>
                            <div class="form-group mb-0"><label>IMEI 2 (اختیاری):</label><input type="text" v-model="form.imei2" class="form-control dir-ltr text-center" maxlength="50"></div>
                        </div>
                    </div>

                    <div v-if="form.paymentType === 'installment'" class="dynamic-section theme-install">
                        <strong class="section-badge">تنظیمات اقساط</strong>
                        <div class="form-group mt-sm">
                            <label class="text-purple font-900" style="color: #6d28d9;">دریافتی علی‌الحساب (پیش‌پرداخت):</label>
                            <div class="input-with-currency">
                                <input type="text" :value="formatNumber(form.downPayment)" @input="e => handleNumberInput('downPayment', e)" class="form-control text-center font-900" dir="ltr" placeholder="0" style="border-color: #c4b5fd; background: #f5f3ff; color: #6d28d9;">
                                <span class="currency-addon" style="color: #6d28d9;">تومان</span>
                            </div>
                        </div>
                        <div class="form-row mt-md">
                            <div class="form-group mb-0"><label>تعداد اقساط:</label><input type="number" min="1" v-model.number="form.installCount" class="form-control text-center dir-ltr"></div>
                            <div class="form-group mb-0"><label>سود ماهانه (%):</label><input type="number" step="0.1" v-model.number="form.installRate" class="form-control text-center dir-ltr"></div>
                        </div>
                        
                        <div class="install-result-box mt-md" v-if="calculatedInstallment.principal > 0 && form.installCount > 0" style="background: #f8fafc; border-color: #cbd5e1;">
                            <div class="ir-row mb-xs">
                                <span class="text-muted">مبلغ باقیمانده (بدون سود):</span> 
                                <span class="dir-ltr text-dark">{{ formatNumber(calculatedInstallment.principal) }} تومان</span>
                            </div>
                            <div class="ir-row mb-xs" v-if="form.installRate > 0">
                                <span class="text-muted">سود محاسبه شده هر قسط:</span> 
                                <span class="dir-ltr text-dark">{{ formatNumber(calculatedInstallment.monthlyInterest) }} تومان</span>
                            </div>
                            <div class="ir-row pt-xs mt-xs" style="border-top: 1px dashed #c4b5fd;">
                                <span style="color: #6d28d9; font-weight: 900;">مبلغ دقیق هر قسط:</span> 
                                <span class="dir-ltr font-900" style="font-size: 15px; color: #6d28d9;">
                                    {{ formatNumber(calculatedInstallment.monthlyPayment) }} <small style="font-size:10px;">تومان</small>
                                </span>
                            </div>
                        </div>
                    ...
                    <div class="d-flex gap-md" style="margin-top: 40px !important;">
                        <button type="submit" class="btn btn-primary mt-0 flex-2 shadow-primary">{{ editId ? 'ذخیره تغییرات' : 'ثبت نهایی فاکتور' }}</button>
        ...
        <div v-show="!isFormOpen">
            <div v-if="filteredSales.length === 0" class="empty-state-app">
                <div class="empty-icon">📂</div>
                <p>{{ searchQuery ? 'نتیجه‌ای یافت نشد.' : 'هیچ فروشی در سیستم ثبت نشده است.' }}</p>
            </div>
            ...
            <div v-else class="app-list-wrapper">
                <div v-for="sale in filteredSales" :key="sale.id" 
                     ...
                     :class="{
                         'overdue-highlight': getFinancials(sale).debt > 0 && sale.paymentType !== 'cash',
                         'settled-highlight': getFinancials(sale).debt <= 0 && sale.paymentType !== 'cash'
                     }"
                    ...
                    <div class="sc-header-modern">
                        <div class="sc-right-info">
                            <div class="sc-meta-row mb-sm">
                                <span class="sc-invoice-badge">فاکتور: <span class="dir-ltr d-inline-block">{{ sale.invoiceNumber }}</span></span>
                                <span class="sc-date-text dir-ltr">{{ sale.date }}</span>
                            </div>
                            <strong class="sc-name-text mb-xs">{{ sale.customerName || 'مشتری عمومی' }}</strong>
                            <div class="sc-item-box">{{ sale.itemName }}</div>
                        </div>
                        
                        <div class="sc-left-badges">
                            <span class="sc-badge-type" :class="sale.paymentType === 'cash' ? 'type-cash' : (sale.paymentType === 'credit' ? 'type-credit' : 'type-installment')">
                                {{ sale.paymentType === 'cash' ? 'نقدی' : (sale.paymentType === 'credit' ? 'نسیه' : 'اقساطی') }}
                            </span>
                            <span class="sc-badge-status mt-xs" :class="getFinancials(sale).debt <= 0 ? 'status-settled' : 'status-debt'">
                                <svg v-if="getFinancials(sale).debt <= 0" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>
                                ...
                                {{ getFinancials(sale).debt <= 0 ? 'تسویه کامل' : (sale.paymentType === 'installment' ? 'اقساط فعال' : 'دارای مطالبات') }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="sale-card-footer-grid">
                        <div class="sale-footer-col">
                            <span class="app-footer-label">مبلغ فاکتور</span>
                            <div class="d-flex align-center justify-center mt-sm" style="direction: rtl; gap: 4px;">
                                <strong class="app-footer-value text-dark dir-ltr">{{ formatNumber(sale.sellPrice) }}</strong>
                                <span class="currency-label text-muted">تومان</span>
                            </div>
                        </div>
                        <div class="sale-footer-divider"></div>
                        <div class="sale-footer-col">
                            <span class="app-footer-label">سود محقق‌شده</span>
                            <div class="d-flex align-center justify-center mt-sm" style="direction: rtl; gap: 4px;">
                                <strong class="app-footer-value dir-ltr" :class="getFinancials(sale).realizedProfit >= 0 ? 'text-success-dark' : 'text-danger-dark'">
                                    {{ formatNumber(getFinancials(sale).realizedProfit) }}
                                </strong>
                                <span class="currency-label" :class="getFinancials(sale).realizedProfit >= 0 ? 'text-success-dark' : 'text-danger-dark'">تومان</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <button v-show="!isFormOpen" @click="openForm" class="fab-new-sale" title="ثبت فروش جدید" style="background: linear-gradient(135deg, #3b82f6, #2563eb);">
            ...
            <div class="modern-bottom-sheet">
                ...
                <div class="modal-top-bar px-sm mb-md">
                    <div class="d-flex align-center gap-sm">
                        ...
                        <div>
                            <h3 class="m-0 text-dark" style="font-size: 17px; font-weight: 900;">گزارش مالی فروش</h3>
                    ...
                    <div class="stats-vertical-layout">
                        ...
                        <div class="stat-box-pro bg-primary-light border-primary text-center">
                            <div class="stat-header text-primary-dark justify-center">
                                ...
                                <span>مبلغ کل فاکتورها</span>
                            </div>
                            <div class="stat-value-container text-primary-dark">
                                <strong class="dir-ltr">{{ formatNumber(stats.totalSalesAmount) }}</strong>
                                <span class="currency-std">تومان</span>
                            </div>
                            <div class="stat-tags-container mt-md">
                                <div class="stat-tag-item"><span>نقدی</span><strong>{{ stats.cashCount }}</strong></div>
                                <div class="stat-tag-divider"></div>
                                <div class="stat-tag-item"><span>نسیه</span><strong>{{ stats.creditCount }}</strong></div>
                                <div class="stat-tag-divider"></div>
                                <div class="stat-tag-item"><span>قسطی</span><strong>{{ stats.installCount }}</strong></div>
                            </div>
                        </div>
                            ...
                            <div class="d-flex w-100 align-center">
                                <div class="flex-1 text-center p-md" style="border-left: 1px dashed #a7f3d0;">
                                    <div class="stat-header text-success-dark justify-center mb-xs">
                                        ...
                                        <span style="font-size: 11px;">سود محقق‌شده</span>
                                    </div>
                        ...
                        <div class="stats-horizontal-row">
                            <div class="stat-box-small bg-danger-light border-danger text-center">
                                <div class="stat-header text-danger-dark justify-center">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
                                    <span>مطالبات از مشتریان</span>
                                </div>
                                <div class="stat-value-container text-danger-dark">
                                    <strong class="dir-ltr" style="font-size: 20px;">{{ formatNumber(stats.creditDebt) }}</strong>
                            ...
                            <div class="stat-box-small bg-warning-light border-warning text-center">
                                ...
                                <div class="stat-value-container text-warning-dark">
                                    <strong class="dir-ltr" style="font-size: 20px;">{{ formatNumber(stats.installmentDebt) }}</strong>
                                    <span class="currency-std" style="font-size: 11px;">تومان</span>
                                </div>
                                <span class="stat-sub text-warning-dark opacity-80" style="font-size: 10px; margin-top: 6px; display: block; text-align: center;">({{ stats.activeInstallments }} پرونده باز)</span>
                            </div>
                        </div>

                        <div class="stat-box-pro bg-slate-light border-slate text-center">
                            ...
                            <div class="stat-value-container text-slate-dark m-0">
                                <strong class="dir-ltr" style="font-size: 22px;">{{ formatNumber(stats.totalPartnerShare) }}</strong>
                                <span class="currency-std">تومان</span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="modern-bottom-sheet">
                ...
                <div class="modern-sheet-body hide-scroll custom-scroll sheet-body-padded">
                    ...
                    <div class="modal-profile-header-new">
                        <div class="mph-top-row">
                            <div class="mph-meta-group">
                                <span class="mph-invoice-badge">فاکتور: <span class="dir-ltr d-inline-block">{{ activeSale?.invoiceNumber }}</span></span>
                                <span class="mph-date"><span class="dir-ltr d-inline-block">{{ activeSale?.date }}</span></span>
                            </div>
                        ...
                        <div class="mph-customer-row">
                            <div class="mph-avatar">{{ activeSale?.customerName?.charAt(0) || 'م' }}</div>
                            <div class="mph-customer-details">
                                <h3 class="mph-name">{{ activeSale?.customerName || 'مشتری عمومی' }}</h3>
                                <span v-if="activeSale?.phone" class="mph-phone dir-ltr">
                        ...
                        <div class="mph-item-box">
                            <strong class="mph-item-name">{{ activeSale?.itemName }}</strong>
                        </div>
                    </div>

                    <div class="hero-balance-section" style="margin-bottom: 24px;" v-if="activeSale?.paymentType !== 'cash' && getFinancials(activeSale).debt > 0">
                        <span class="hero-label">{{ activeSale?.paymentType === 'installment' ? 'مانده اقساط' : 'مطالبات از مشتری' }}</span>
                        <div class="hero-amount-container text-danger-dark">
                            <strong class="hero-amount dir-ltr">{{ formatNumber(getFinancials(activeSale).debt) }}</strong><span class="hero-currency">تومان</span>
                        </div>
                    </div>
                    ...
                    <div class="solid-stats-grid mb-md">
                        <div class="ss-item ss-border-b ss-border-l"><span class="ss-lbl">هزینه اصلی کالا</span><strong class="ss-val dir-ltr">{{ formatNumber(getFinancials(activeSale).cost) }}</strong></div>
                        <div class="ss-item ss-border-b ss-border-l"><span class="ss-lbl">مبلغ فاکتور</span><strong class="ss-val dir-ltr">{{ formatNumber(activeSale?.sellPrice) }}</strong></div>
                        <div class="ss-item ss-border-b"><span class="ss-lbl">علی‌الحساب</span><strong class="ss-val text-dark dir-ltr">{{ formatNumber(getInitialPayment(activeSale)) }}</strong></div>
                        <div class="ss-item bg-lightest ss-border-l"><span class="ss-lbl">نوع حساب</span><strong class="ss-val" :class="activeSale?.paymentType === 'cash' ? 'text-success-dark' : 'text-warning-dark'">{{ getPaymentTypeName(activeSale?.paymentType) }}</strong></div>
                        <div class="ss-item bg-lightest ss-border-l"><span class="ss-lbl">سود دفتری</span><strong class="ss-val text-dark dir-ltr">{{ formatNumber(getFinancials(activeSale).expectedProfit) }}</strong></div>
                        <div class="ss-item bg-lightest"><span class="ss-lbl">سود محقق‌شده</span><strong class="ss-val text-success-dark dir-ltr" :class="{'text-danger-dark': getFinancials(activeSale).realizedProfit < 0}">{{ formatNumber(getFinancials(activeSale).realizedProfit) }}</strong></div>
                    </div>

                    <div v-if="activeSale?.paymentType === 'installment'">
                        <div class="section-title-mod" style="color: #6d28d9; margin-top: 24px;"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="5" width="20" height="14" rx="2"></rect><line x1="2" y1="10" x2="22" y2="10"></line></svg> جزئیات مالی اقساط</div>
                        <div class="solid-stats-grid mb-md" style="border-color: #ddd6fe; box-shadow: 0 4px 10px rgba(109, 40, 217, 0.05);">
                            <div class="ss-item ss-border-b ss-border-l"><span class="ss-lbl">مبلغ هر قسط</span><strong class="ss-val text-primary-dark dir-ltr">{{ formatNumber(getFinancials(activeSale).paymentPerInst) }}</strong></div>
                            <div class="ss-item ss-border-b ss-border-l"><span class="ss-lbl">تعداد اقساط</span><strong class="ss-val">{{ activeSale?.installCount }} ماهه</strong></div>
                            <div class="ss-item ss-border-b"><span class="ss-lbl">درصد ماهانه</span><strong class="ss-val dir-ltr">{{ activeSale?.rate || 0 }}٪</strong></div>
                        </div>
                        <div class="text-center text-xs text-muted mb-md p-md" style="background: #f5f3ff; border-radius: 12px; border: 1px dashed #c4b5fd;">مدیریت وضعیت اقساط و ریزپرداخت‌ها از طریق <strong>تب «اقساط»</strong> انجام می‌شود.</div>
                    </div>

                    <button v-if="activeSale?.paymentType === 'credit'" class="btn-guaranty-toggle-centered mb-lg mt-md" :class="{'has-guaranty': activeSale?.guaranty?.name || activeSale?.guaranty?.chequeNumber}" @click="handleGuarantyClick">
                        ...
                        <span class="badge-status-guaranty has-data" v-if="activeSale?.guaranty?.name || activeSale?.guaranty?.chequeNumber">
                    ...
                    <!-- فقط IMEI 1 و 2 تحت عنوان اطلاعات تکمیلی -->
                    <div v-if="activeSale?.imei1 || activeSale?.imei2">
                        <div class="section-title-mod" style="margin-top: 16px;">اطلاعات تکمیلی</div>
                        <div class="mb-sm" style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 12px;">
                            <div v-if="activeSale?.imei1" style="display:flex; justify-content:space-between; font-size:12px; margin-bottom:6px;"><span class="text-muted">IMEI 1:</span> <strong class="dir-ltr text-dark">{{ activeSale.imei1 }}</strong></div>
                            <div v-if="activeSale?.imei2" style="display:flex; justify-content:space-between; font-size:12px;"><span class="text-muted">IMEI 2:</span> <strong class="dir-ltr text-dark">{{ activeSale.imei2 }}</strong></div>
                        </div>
                    </div>
                    
                    <!-- یادداشت‌ها مجزا نمایش داده شوند -->
                    <div v-if="activeSale?.notes" class="mb-md mt-md" style="background: var(--warning-light); border: 1px solid #fde68a; border-radius: 12px; padding: 14px;">
                        <strong class="text-sm text-warning-dark d-block mb-xs">یادداشت‌ها:</strong>
                        <p class="text-muted text-sm m-0" style="line-height: 1.6;">{{ activeSale.notes }}</p>
                    </div>

                    <div class="quick-pay-box mb-md" v-if="activeSale?.paymentType === 'credit' && getFinancials(activeSale).debt > 0">
                    ...
                    <div class="payments-timeline-wrapper" v-if="activeSale?.paymentType === 'credit' && activeSale?.payments?.length > 0">
                        <div class="timeline-header">
                            <div class="th-icon"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg></div>
                            <h4 class="th-title">تاریخچه واریزی‌های نسیه</h4>
                            <div class="th-badge">{{ activeSale.payments.length }} مورد</div>
                        </div>
                        <div class="timeline-list">
                            <div v-for="(p, index) in activeSale.payments" :key="p.id" class="tl-item">
                    ...
                    <div class="d-flex gap-sm mt-xl pt-xl" style="border-top: 1px dashed #cbd5e1; margin-top: 36px; padding-top: 24px;">
                        <button class="btn btn-primary flex-1 m-0" style="background: #eff6ff; color: #2563eb; border-radius: 12px; box-shadow: none;" @click="editSale(activeSale)">
                        ...
                        <button class="btn btn-danger flex-1 m-0" style="background: #fff1f2; color: #e11d48; border-radius: 12px; box-shadow: none;" @click="deleteSale(activeSale.id)">
                ...
                <div class="modern-sheet-body hide-scroll px-md pb-xl">
                    <div class="guaranty-card-premium mb-md">
                        <div class="g-card-header pb-sm mb-sm border-bottom-dashed">
                            ...
                            <span class="text-xs text-muted dir-ltr">{{ activeSale?.guaranty?.date || 'بدون تاریخ' }}</span>
                        </div>

                        <div class="g-info-grid">
                            <div class="g-info-item" v-if="activeSale?.guaranty?.name">
                                ...
                                <strong class="g-val">{{ activeSale.guaranty.name }}</strong>
                            </div>
                            <div class="g-info-item" v-if="activeSale?.guaranty?.phone">
                                ...
                                <strong class="g-val dir-ltr text-left">{{ activeSale.guaranty.phone }}</strong>
                            </div>
                            <div class="g-info-item" v-if="activeSale?.guaranty?.bank">
                                <span class="g-lbl"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg> بانک صادرکننده</span>
                                <strong class="g-val">{{ activeSale.guaranty.bank }}</strong>
                            </div>
                            <div class="g-info-item" v-if="activeSale?.guaranty?.chequeNumber">
                                <span class="g-lbl"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg> شماره چک/صیاد</span>
                                <strong class="g-val dir-ltr text-left">{{ activeSale.guaranty.chequeNumber }}</strong>
                            </div>
                            
                            <div class="g-info-item" v-if="activeSale?.guaranty?.amount">
                                ...
                                <div class="d-flex align-baseline gap-xs" style="direction: rtl; justify-content: flex-end;">
                                    <strong class="g-val dir-ltr text-dark">{{ formatNumber(activeSale.guaranty.amount) }}</strong>
                                    <span style="font-size: 10px; font-weight: 700; color: #64748b;">تومان</span>
                                </div>
                            </div>
                        </div>

                        <div class="g-info-item full-width mt-sm" v-if="activeSale?.guaranty?.description" style="background: #fffbeb; border-color: #fde68a;">
                            ...
                            <strong class="g-val" style="line-height: 1.6; font-size: 11.5px; color: #78350f;">{{ activeSale.guaranty.description }}</strong>
                        </div>
                    </div>
...
export default {
    setup() {
        const { formatNumber, unformatNumber, getFinancials, getInitialPayment, getPaymentTypeName, getPersianDate, addMonthsPersian, toEnglishDigits } = window.SysUtils;
        ...
        const form = reactive({
            date: '', customerName: '', phone: '', itemName: '', notes: '',
            buyPrice: '', sellPrice: '', partnerShare: '', paymentType: 'cash',
            initialPayment: '', downPayment: '', installCount: '', installRate: '', installInterval: 1,
            imei1: '', imei2: ''
        });
        ...
        const loadData = async () => {
            if (window.DB) {
                sales.value = await window.DB.getAll('sales');
                sales.value.sort((a, b) => b.timestamp - a.timestamp);
                
                if (activeSale.value) {
                    const updatedSale = sales.value.find(s => s.id === activeSale.value.id);
                    if (updatedSale) activeSale.value = updatedSale;
                }
            }
        };
        ...
        const filteredSales = computed(() => {
            ...
            if (searchQuery.value) {
                ...
                result = result.filter(sale => 
                    ...
                    (sale.itemName && sale.itemName.toLowerCase().includes(q)) ||
            ...
            if (currentFilter.value === 'custom') {
                return result.filter(sale => {
                    ...
                    let match = true;
                    if (customStartDate.value && sDate < customStartDate.value) match = false;
                    if (customEndDate.value && sDate > customEndDate.value) match = false;
                    return match;
                });
            }
        ...
        const stats = computed(() => {
            ...
            let cashCount = 0, creditCount = 0, installCount = 0, activeInstallments = 0;

            filteredSales.value.forEach(s => {
                const fin = getFinancials(s);
                totalSalesAmount += fin.finalPrice;
                totalRealizedProfit += fin.realizedProfit; 
                totalExpectedProfit += fin.expectedProfit; 
                totalPartnerShare += Number(s.partnerShare || 0);
                ...
                if (s.paymentType === 'cash') cashCount++;
                    ...
                    if (fin.debt > 0) {
                        activeInstallments++;
                        installmentDebt += fin.debt;
                    }
                }
            });

            return { 
                totalSalesAmount, totalRealizedProfit, totalExpectedProfit, totalUnrealizedProfit,
                cashCount, creditCount, installCount, 
                activeInstallments, creditDebt, installmentDebt, totalPartnerShare 
            };
        });

        const calculatedInstallment = computed(() => {
            const sellPrice = Number(form.sellPrice) || 0;
            const downPay = Number(form.downPayment) || 0;
            const count = Number(form.installCount) || 0;
            const rate = Number(form.installRate) || 0;

            if (count > 0 && sellPrice > 0 && sellPrice >= downPay) {
                const principal = sellPrice - downPay;
                const monthlyInterest = Math.round(principal * (rate / 100));
                const baseAmount = Math.round(principal / count);
                const monthlyPayment = baseAmount + monthlyInterest;
                
                return { monthlyPayment, principal, monthlyInterest };
            }
            return { monthlyPayment: 0, principal: 0, monthlyInterest: 0 };
        });

        const openForm = () => {
            Object.assign(form, {
                date: getPersianDate(), customerName: '', phone: '', itemName: '', notes: '',
                buyPrice: '', sellPrice: '', partnerShare: '', paymentType: 'cash',
                initialPayment: '', downPayment: '', installCount: '', installRate: '', installInterval: 1,
                imei1: '', imei2: ''
            });
        ...
        const saveSale = async () => {
            try {
                if (form.paymentType === 'installment' && Number(form.downPayment) >= Number(form.sellPrice)) {
                    alert('مبلغ دریافتی علی‌الحساب نمی‌تواند بیشتر یا مساوی مبلغ فاکتور باشد.');
                    return;
                }
                if (form.paymentType === 'credit' && Number(form.initialPayment) >= Number(form.sellPrice)) {
                    alert('مبلغ دریافتی علی‌الحساب نمی‌تواند بیشتر یا مساوی مبلغ فاکتور باشد.');
                    return;
                }

                const existingSale = editId.value ? sales.value.find(s => s.id === editId.value) : null;
                const baseData = existingSale ? JSON.parse(JSON.stringify(existingSale)) : { id: window.DB.generateId(), timestamp: Date.now(), payments: [] };

                baseData.invoiceNumber = existingSale?.invoiceNumber || await window.DB.getNextInvoiceNumber();
                
                baseData.itemName = form.itemName;
                baseData.customerName = form.customerName || 'مشتری عمومی';
                baseData.date = toEnglishDigits(form.date);
                baseData.phone = toEnglishDigits(form.phone);
                baseData.notes = form.notes;
                baseData.buyPrice = Number(form.buyPrice) || 0;
                baseData.sellPrice = Number(form.sellPrice) || 0;
                baseData.partnerShare = Number(form.partnerShare) || 0;
                baseData.paymentType = form.paymentType;
                
                baseData.profit = baseData.sellPrice - baseData.buyPrice - baseData.partnerShare;

                baseData.imei1 = form.paymentType !== 'cash' ? form.imei1 : '';
                baseData.imei2 = form.paymentType !== 'cash' ? form.imei2 : '';

                if (form.paymentType === 'cash') {
                    baseData.initialPayment = 0;
                    baseData.downPayment = 0;
                    baseData.installCount = 0;
                    ...
                    baseData.initialPayment = 0;
                    baseData.downPayment = Number(form.downPayment) || 0;
                    baseData.installCount = Number(form.installCount) || 0;
                    baseData.installInterval = Number(form.installInterval) || 1;
                    baseData.rate = Number(form.installRate) || 0;
                    
                    const needsNewSchedule = !existingSale || 
                                             existingSale.paymentType !== 'installment' || 
                                             existingSale.installCount !== baseData.installCount ||
                                             existingSale.sellPrice !== baseData.sellPrice ||
                                             existingSale.downPayment !== baseData.downPayment ||
                                             existingSale.rate !== baseData.rate ||
                                             !baseData.schedule || baseData.schedule.length === 0;

                    if (needsNewSchedule) {
                        if (existingSale && existingSale.paymentType === 'installment' && existingSale.payments && existingSale.payments.length > 0) {
                            if (!confirm("تغییرات شما باعث بازنویسی تقویم اقساط می‌شود. ادامه می‌دهید؟")) return;
                        }

                        baseData.schedule = [];
                        const principal = baseData.sellPrice - baseData.downPayment;
                        const count = baseData.installCount;
                        const rate = baseData.rate;
                        
                        if (count > 0) {
                            const monthlyInterest = Math.round(principal * (rate / 100));
                            const baseAmount = Math.round(principal / count);
                            const amountPerInst = baseAmount + monthlyInterest;
                            const totalDebtWithInterest = amountPerInst * count;
                            
                            let currentDate = baseData.date;
                            
                            for (let i = 0; i < count; i++) {
                                currentDate = addMonthsPersian(currentDate, baseData.installInterval || 1);
                                let amt = amountPerInst;
                                if (i === count - 1) amt = totalDebtWithInterest - (amountPerInst * (count - 1));
                                
                                baseData.schedule.push({
                                    id: Date.now() + i,
                                    amount: amt,
                                    date: currentDate,
                                    isPaid: false,
                ...
                await window.DB.put('sales', baseData);
                closeForm();
                await loadData();
            } catch (err) {
        ...
        const editSale = (sale) => {
            ...
            Object.assign(form, {
                date: sale.date, customerName: sale.customerName, phone: sale.phone, itemName: sale.itemName, notes: sale.notes,
                buyPrice: sale.buyPrice, sellPrice: sale.sellPrice, partnerShare: sale.partnerShare, paymentType: sale.paymentType,
                initialPayment: sale.initialPayment || '', downPayment: sale.downPayment || '', 
                installCount: sale.installCount || '', installRate: sale.rate || '', installInterval: sale.installInterval || 1,
                imei1: sale.imei1 || '', imei2: sale.imei2 || ''
            });
            isFormOpen.value = true;
        };
        ...
        const handleGuarantyClick = () => {
            if (activeSale.value?.guaranty?.name || activeSale.value?.guaranty?.chequeNumber) {
        ...
        const openGuarantyForm = () => {
            const g = activeSale.value.guaranty || {};
        ...
        const saveGuaranty = async () => {
            const sale = JSON.parse(JSON.stringify(activeSale.value));
            sale.guaranty = {
        ...
        const deletePayment = async (saleId, paymentId) => {
            if (!confirm("آیا از حذف این دریافتی اطمینان دارید؟")) return;
            const sale = JSON.parse(JSON.stringify(sales.value.find(s => s.id === saleId)));
        ...
        return {
            sales, isFormOpen, editId, form, currentFilter, searchInput, searchQuery, onSearchInput, clearSearch, filters, filteredSales, 
            calculatedInstallment, stats, showDetailsModal, activeSale, quickPayAmount,
            showStatsModal, showFilterModal, showCustomDateModal, customStartDate, customEndDate,
            showGuarantyViewModal, showGuarantyModal, guarantyForm,
            filterLabel, applyFilter, applyCustomFilter, handleNumberInput, handleQuickPayInput, handleGuarantyAmountInput,
            formatNumber, unformatNumber, openForm, closeForm, saveSale, deleteSale, editSale, 
            openDetails, addPayment, deletePayment, getFinancials,
            getInitialPayment, getPaymentTypeName, handleGuarantyClick, openGuarantyForm, saveGuaranty, deleteGuaranty
        };
    }
}
</script>
...
.stats-banner-card { flex: 1; border-radius: 16px; padding: 14px 16px; display: flex; align-items: center; justify-content: space-between; cursor: pointer; color: white; box-shadow: 0 6px 15px rgba(37, 99, 235, 0.25); transition: var(--transition); }
...
/* ------ استایل‌های یکپارچه کارت‌های لیست فروش ------ */
...
.modern-form .form-control:focus { border-color: var(--primary); box-shadow: 0 0 0 3px var(--primary-light); }
...
.payment-type-selector { display: flex; gap: 8px; background: var(--surface); padding: 8px; border-radius: 16px; border: 1px solid var(--border); box-shadow: inset 0 2px 4px rgba(0,0,0,0.02); }
.pt-option { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 12px 4px; gap: 8px; border-radius: 12px; cursor: pointer; transition: all 0.25s ease; color: var(--text-muted); font-size: 11px; font-weight: 800; border: 1px solid transparent; background: transparent; }
.pt-option svg { opacity: 0.5; transition: all 0.25s ease; transform: scale(0.95); }
.pt-option.active-cash { background: #ecfdf5; color: var(--success-dark); border-color: #a7f3d0; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.15); }
.pt-option.active-cash svg { opacity: 1; transform: scale(1.1); color: var(--success-dark); }
.pt-option.active-credit { background: #fffbeb; color: #b45309; border-color: #fde68a; box-shadow: 0 4px 12px rgba(245, 158, 11, 0.15); }
.pt-option.active-credit svg { opacity: 1; transform: scale(1.1); color: #d97706; }
.pt-option.active-install { background: #eff6ff; color: #1d4ed8; border-color: #bfdbfe; box-shadow: 0 4px 12px rgba(37, 99, 235, 0.15); }
.pt-option.active-install svg { opacity: 1; transform: scale(1.1); color: #2563eb; }

.install-result-box { background: white; border: 1px solid #ddd6fe; border-radius: 12px; padding: 12px 14px; display: flex; flex-direction: column; gap: 8px; border-color: #bbf7d0; }
...
.fab-new-sale { position: fixed; bottom: 115px; left: 20px; z-index: 900; width: 56px; height: 56px; border-radius: 18px; color: white; border: none; display: flex; align-items: center; justify-content: center; box-shadow: 0 10px 25px -5px rgba(37, 99, 235, 0.5); cursor: pointer; transition: var(--transition); }
.fab-new-sale:active { transform: scale(0.9) translateY(4px); box-shadow: 0 4px 10px rgba(37, 99, 235, 0.4); }

.stat-tags-container { display: flex; align-items: center; background: #ffffff; border-radius: 14px; padding: 10px 8px; width: 100%; box-shadow: 0 2px 8px rgba(37, 99, 235, 0.08); border: 1px solid #bfdbfe; margin-top: 12px; }
.stat-tag-item { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 2px; }
.stat-tag-item span { font-size: 10.5px; font-weight: 800; color: #64748b; }
.stat-tag-item strong { font-size: 15px; font-weight: 900; color: #1e40af; margin-top: 4px; line-height: 1; }
.stat-tag-divider { width: 1px; height: 28px; background: #e2e8f0; border-radius: 2px; }
